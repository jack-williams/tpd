    ...
    if (member instanceof TypeScript.FunctionDeclaration) {
    	...
    	if (member.isEnum()) { // enums are ModuleDeclarations in the AST, but they are semantically quite different
            ...
            if (name[0] === '"' || name[0] === "'") { // external module?
                ...
                var cleaned_name = name.substring(1, name.length-1) // remove quotes
                parseModule(member, submodule, qualify(qname, cleaned_name),true)    
            } else {
                var submodule = moduleObject.getModule(name)
        ...
        if (topLevel || TypeScript.hasFlag(member.getVarFlags(), TypeScript.VariableFlags.Exported)) {
            moduleObject.types.put(member.id.text(), ref)
        } else {
            // private alias to (potentially) publicly visible type
            current_scope.env.put(member.id.text(), ref) 
        }
    }
        ...
        extern_types.put(qname, lookup(current_scope, member.id.text()), current_scope);

        //extern_types.put(name, resolve(), current_scope)
    }
    else {
...
function parseModule(node, moduleObject, qname, external) {
    moduleObject.qname = 'module:' + qname;
    moduleObject.qname = external ? 'external_' + moduleObject.qname : moduleObject.qname;
...
function parseType(node) {
    ...
    if (node instanceof TypeScript.GenericType) {
        ...
        var requoted = node.text();
        if (requoted[0] === "'" || requoted[0] === '"') {
            requoted = requoted.substring(1, requoted.length-1);
            requoted = '"' + requoted + '"';
        }
    	// try to resolve early (type parameters must be resolved before merging)
    	var t = lookupTypeParameter(current_scope, requoted)
    	if (t) {
    	    return t;
    	}
    	// defer resolution for later
        return new TReference(requoted, current_scope)
    }
...
function lookup(scope, name,isModule) {
    while (scope !== null) {
        var t = lookupPrtyInScopeDirect(scope,name) ||
            lookupInScopeDirect(scope,name,isModule);
        if (t)
            return t;
        scope = scope.parent
    }
...
function outputType(type) {
    if (type instanceof TObject) {
      //console.log(type.calls.map(outputCall))
        return {
            type: 'object',
        ...
        return {type: 'reference', name:type.qname, typeArguments:[], meta: {origin: current_file}}
    }
    else if (type instanceof TTypeParam) {
        return {type: 'type-param', name:type.name, meta: {origin: current_file}}
    }
    else if (type instanceof TGeneric)  {
        ...
        return {
            ...
            meta: {origin: current_file}
        }
    }
    else if (type instanceof TBuiltin) {
        return { type: type.name, meta: {origin: current_file} }
    }
    else if (type instanceof TEnum) {
        return { type: 'enum', name: type.qname, meta: {origin: current_file} }
    }
    else if (type instanceof TString) {
        return { type: 'string-const', value: type.value, meta: {origin: current_file} }
    }
    else {
...
function outputExtern(t) {
  // JACK EDIT: We also want to be able to output modules in some cases
  // declare module f {
  ...
  if (t instanceof TQualifiedReference)
  {
    var meta;
    try {
      var found = lookupQualifiedType(t);
      meta = found.meta
    } catch (e) {
      meta = {origin: current_file};
    }
    ...
    return {
      type: 'object',
      // typeParameters: type.typeParameters.map(outputTypeParameter),
      properties: t.properties.mapv(outputProperty).json(),
      calls: t.calls.map(outputCall).compact(),
      stringIndexer: findIndexer(t.calls, 'string'),
      numberIndexer: findIndexer(t.calls, 'number'),
      brand: t.brand,
      meta: t.meta
    }
  }
  else if (t instanceof TBuiltin) {
    return { type: t.name, meta: {origin: current_file}}
  } else if (t instanceof TGeneric)  {
    if (!(t.base instanceof TQualifiedReference)) {
      throw new Error("Malformed TGeneric base in: " + util.inspect(t))
    }
    return {
      type: 'reference',
      name: t.base.qname,
      typeArguments: t.args.map(outputType),
      meta: {origin: current_file}
    }
  } else {
...
function convert(arg) {
  ...
  inputs = inputs.map(function(input) {
    ...
    // JACK EDIT: Re-set incase we want to call convert multiple times in a session.
    enum_types = new Map; 
